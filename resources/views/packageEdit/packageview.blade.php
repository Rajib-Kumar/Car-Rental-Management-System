							...
							<div class="col-lg-12">
								...
								<div class="well">
									<table align="center" class="table table-bordered" style="background-color: white;">
										...
										@foreach($pcklist as $pcklist)
										<tr>
											<td>{{$pcklist->packageId}}</td>
											<td>{{$pcklist->rentalType}}</td>
											<td>{{$pcklist->vehicleType}}</td>
											<td>{{$pcklist->location}}</td>
											<td>{{$pcklist->price}}</td>
											
											<td><a href="/package/{{$pcklist->packageId}}">Details</a></td>
											<td><a href="/package/{{$pcklist->packageId}}/edit">Edit</a></td>
										</tr>
										@endforeach
									</table>
								</div>
